...
export default function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errors) {
        ...
        setError(Array.isArray(firstError) ? firstError[0] : errorMsg);
      } else {
        setError(errorMsg);
      }
    }
  ...
  return (
    <div className="w-full max-w-[480px] rounded-[10px] bg-white p-7.5 shadow-card dark:bg-gray-dark dark:shadow-card-dark sm:p-12.5 xl:px-17.5">
      <h1 className="mb-2.5 text-3xl font-black text-dark dark:text-white sm:text-4xl">
        Create Account
      </h1>
      <p className="mb-7.5 font-medium">
        Sign up to get started with your trainee account
      </p>

      {error && (
        <div className="mb-5 rounded-lg bg-red-100 p-4 text-sm text-red-800 dark:bg-red-900/30 dark:text-red-300">
          {error}
        </div>
      )}
        ...
        <div className="mb-5">
          ...
          <div className="relative">
            <input
              ...
              className="w-full rounded-lg border border-stroke bg-transparent py-3.5 px-5 pr-12 font-medium text-dark outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-2 dark:border-dark-3 dark:bg-dark-2 dark:text-white dark:focus:border-primary"
